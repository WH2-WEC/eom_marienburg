local BRET_PROTECTION_BUNDLE = "SAM_ADD" 
local EMP_PROTECTION_BUNDLE = "SAM_ADD"
local PROTECTION_TECH = "SAM_ADD"
...
core:add_listener(
    ...
    function(context)
        if cm:get_faction("wh_main_emp_marienburg"):has_technology(PROTECTION_TECH) or MARIENBURG_TESTING_VAR == true then
            if context:proposer():subculture() == "wh_main_sc_emp_empire" then
                cm:apply_effect_bundle(EMP_PROTECTION_BUNDLE, "wh_main_emp_marienburg", 0)
            ...
            elseif context:proposer():subculture() == "wh_main_sc_brt_bretonnia" then
                cm:apply_effect_bundle(BRET_PROTECTION_BUNDLE, "wh_main_emp_marienburg", 0)
...
core:add_listener(
    ...
    function(context)
        ...
        if not marienburg_at_war_with("wh_main_sc_emp_empire") then
            if marienburg:has_effect_bundle(EMP_PROTECTION_BUNDLE) then
                marien:log("Not at war with Empire anymore, removing the protection bundle!")
                cm:remove_effect_bundle(EMP_PROTECTION_BUNDLE, marienburg:name())
            end
        end
        if not marienburg_at_war_with("wh_main_sc_brt_bretonnia") then
            if marienburg:has_effect_bundle(BRET_PROTECTION_BUNDLE) then
                marien:log("Not at war with bretonnia anymore, removing the protection bundle!")
                cm:remove_effect_bundle(BRET_PROTECTION_BUNDLE, marienburg:name())
            end
        end
    end,
    true)
